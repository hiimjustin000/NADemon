...
class $modify(NDLevelInfoLayer, LevelInfoLayer) {
    bool init(GJGameLevel* level, bool challenge) {
        ...
        if (level->m_stars == 0 && level->m_starsRequested == 10) m_difficultySprite->updateDifficultyFrame(6, GJDifficultyName::Short);
    ...
    void likedItem(LikeItemType type, int id, bool liked) {
        ...
        if (m_level->m_stars == 0 && m_level->m_starsRequested == 10) m_difficultySprite->updateDifficultyFrame(6, GJDifficultyName::Short);
    }
};
...
class $modify(NDLevelCell, LevelCell) {
    void loadFromLevel(GJGameLevel* level) {
        ...
        if (auto difficultyContainer = m_mainLayer->getChildByID("difficulty-container")) {
            if (level->m_stars == 0 && level->m_starsRequested == 10)
